...
├── README.md               Bu dosya
├── .env.example            Otomasyon için örnek ortam değişkenleri
├── .gitignore
├── assets/
│   ├── css/style.css       Tüm stiller
│   └── js/
│       ├── config.js       Ayarlar + skor ağırlıkları/eşikleri
│       ├── format.js       Ortak biçimlendirme yardımcıları (num/pct/fx/cap)
│       ├── api.js          Finnhub API istemcisi (tarayıcı)
│       ├── analysis.js     Analiz motoru (saf hesaplama)
│       ├── ui.js           Görselleştirme
│       └── app.js          Olay yönetimi / orkestrasyon
└── automation/             Zamanlanmış tarama + Telegram uyarı botu
    └── README.md           Kurulum ve kullanım (bkz. bu dosya)
```
