/* ============================================================
   ...
   Tarayıcıda config.js + format.js'in tanımladığı global
   CONFIG/fx/pct/num'u kullanır; Node'da (otomasyon scripti)
   require ile aynı isimleri globalThis'e taşır.
   ============================================================ */
if (typeof CONFIG === "undefined" && typeof require === "function") {
  globalThis.CONFIG = require("./config.js");
  Object.assign(globalThis, require("./format.js"));
}
...
const Analyzer = (() => {
  ...
  // Bir metric objesinden getiri merdiveni + temettü verimini çıkarır.
  // analyze()'in kendisi ve benchmark (ör. SPY) hesaplaması bu fonksiyonu
  // paylaşır — alan adları tek yerde tanımlı kalır.
  function returnsFromMetric(m) {
    return {
      d5: pick(m, ["5DayPriceReturnDaily"]),
      m1: pick(m, ["monthToDatePriceReturnDaily"]),
      m3: pick(m, ["13WeekPriceReturnDaily"]),
      m6: pick(m, ["26WeekPriceReturnDaily"]),
      ytd: pick(m, ["yearToDatePriceReturnDaily"]),
      y1: pick(m, ["52WeekPriceReturnDaily"]),
    };
  }

  function dividendYieldFromMetric(m) {
    return pick(m, ["dividendYieldIndicatedAnnual"]);
  }
  ...
  function analyze(m, q, rec) {
    ...
    // Temettü verimi (yıllık, %) — projeksiyonlarda fiyat getirisine ek
    // olarak kullanılır; şirket temettü ödemiyorsa null (0 varsaymak,
    // gerçekte hiç dağıtmayan bir şirketi "en azından bir şey veriyor"
    // gibi göstermemek için önemli).
    const dividendYield = dividendYieldFromMetric(m);

    return {
      overall, verdict, summary, dims, pros, cons, rangePos, hi, lo, recDist, anScore, beta, dividendYield,
      returns: returnsFromMetric(m),
    };
  }

  // Dışarıya açık API
  return { analyze, verdictOf, pick, isNum, returnsFromMetric, dividendYieldFromMetric };
})();
