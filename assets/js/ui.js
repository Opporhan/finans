/* ============================================================
   ...
   (num/pct/fx/cap) format.js'den gelir.
   ============================================================ */
...
const UI = (() => {
  ...
  // Haber başlığı/kaynağı Finnhub'ın aggregate ettiği üçüncü taraf
  // yayıncılardan geliyor — güvenilmeyen metin, innerHTML'e gömülmeden
  // önce escape edilir (bkz. miniapp/public/assets/news.js'teki aynı gerekçe).
  function esc(s) {
  ...
  function renderNews(items) {
    ...
    card.innerHTML = `<h2>Son Haberler</h2>` + items.map((n) => `
      <a class="news-item" href="${esc(n.url)}" target="_blank" rel="noopener">
        ${n.image ? `<img src="${esc(n.image)}" alt="" onerror="this.style.display='none'"/>` : `<div style="width:78px;height:58px;border-radius:9px;background:var(--panel-2);flex-shrink:0;display:grid;place-items:center;color:var(--muted)">📰</div>`}
        <div><div class="news-title">${esc(n.headline)}</div><div class="news-meta">${esc(n.source || "")} · ${new Date(n.datetime * 1000).toLocaleDateString("tr-TR")}</div></div>
      </a>`).join("");
  }
  ...
  function renderSuggestions(items, onPick) {
    ...
    suggestions.innerHTML = items.map((r) => `<div class="sug-item" data-sym="${esc(r.symbol)}"><span class="sym">${esc(r.symbol)}</span><span class="desc">${esc(r.description || "")}</span></div>`).join("");
  ...
  function renderError(code, symbol) {
    const msgs = {
      ...
      NOT_FOUND: `<b>${esc(symbol || "")}</b> için veri bulunamadı. Sembolü kontrol edin (örn: AAPL).`,
    };
